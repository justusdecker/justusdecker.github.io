...
export function MonthlySummaryOverview() {
  const [searchTerm, setSearchTerm] = useState('');

  const shortUrl = "Elekro-Ausbildung-Lernstoff/main/summary/monthly/index.json";
        ...
        {filteredPosts.length > 0 ? (
          filteredPosts.map((post) => (
            <div key={post.id} className="topic">
              <Link to={`/training-electrician/monthly-summary/${post.id}`}>
                <div className="preview-box">
                  <MarkdownLoader 
                    url={`https://raw.githubusercontent.com/justusdecker/Elekro-Ausbildung-Lernstoff/main/summary/monthly/${post.id}.md`} 
                  />
                </div>
              </Link>
